...
-- 2) Which last names appear more than once? We would use the same logic as in the previous question but this time we want to 
...
select count(rental_id)as processed_rentals, staff_id from rental
group by staff_id;
...
select count(title) as released_films, release_year from film
group by release_year;

-- only year 2006: 1000 movies

-- 5) Using the film table, find out for each rating how many films were there.

select count(title) as count, rating from film
group by rating;


-- 6) What is the mean length of the film for each rating type. Round off the average lengths to two decimal places

select round(avg(length),2) as average_lengths, rating from film
group by rating;
